...
function ListCallback(response, startPage) {
  ...
  $.each(response, function(index, value) {
    divHTML += '<div id="'+response[index].groupNo+'" class="img-wrap">' + ' <div class="img-content">'
        ...
        + '<div class="spine spine-right"><button type="button" data-num='+response[index].groupNo+'>홈으로</button></div>'
        + '</div></div>';
...
//    HTML += "</div>";
    ...
    HTML += "<li style='padding-bottom: 10px;'>그룹 활동 기간 : "+ response.groupStartDate +" ~ " + response.groupEndDate + "</li>";
...
function mapDetail(no) {
  ...
  HTML += "<li style='padding-bottom: 10px;'>그룹 활동 기간 : "+ response1.groupStartDate +" ~ " + response1.groupEndDate + "</li>";
  ...
  HTML += "   <li>" + response1.memberName + "</li>";
